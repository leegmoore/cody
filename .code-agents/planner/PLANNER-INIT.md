...
│   └── 02-script-tools/   # Script harness implementation project
...
│   └── test-suites/       # TDD API tests
└── codex-ts/              # Legacy - utilities being migrated (active dependency)
```
...
- `projects/02-script-tools/` - Current project: script-based tool execution
...
**Current project:** `projects/02-script-tools/` - Script-based tool execution
...
1. **`.code-agents/planner/STATE.md`** - Current ground truth (what's working, what's broken)
2. **`.code-agents/planner/CURRENT.md`** - Active slice (what we're working on NOW)
3. **`.code-agents/planner/NEXT.md`** - Work queue (what's coming after current)
4. **`.code-agents/planner/PROCESS.md`** - Workflow and checkpoints (how we work)
