...
describe('OllamaClient', () => {
  ...
  describe('fromBaseUrl', () => {
    ...
    it('should throw error when server is not reachable', async () => {
      ...
      await expect(OllamaClient.fromBaseUrl('http://localhost:11434')).rejects.toThrow(
  ...
  describe('fetchModels', () => {
    ...
    it('should filter out non-string model names', async () => {
      const mockResponse = {
        ...
        status: 200,
        json: async () => ({
          models: [{ name: 'valid' }, { name: 123 }, { name: null }, { other: 'field' }],
        }),
      }
  ...
  describe('pullModelStream', () => {
    ...
    it('should handle error events', async () => {
      ...
      expect(events).toContainEqual({ type: 'error', message: 'Model not found' })
    })
  ...
  describe('pullWithReporter', () => {
    it('should call reporter for each event', async () => {
      const mockBody = {
        getReader: () => {
          let index = 0
          const lines = ['{"status": "downloading"}\n', '{"status": "success"}\n']
      ...
      expect(events.length).toBeGreaterThan(0)
      expect(events[0]).toEqual({ type: 'status', status: 'Pulling model llama3.2:3b...' })
      expect(events[events.length - 1]).toEqual({ type: 'success' })
    })
    ...
    it('should throw error on pull failure', async () => {
      ...
      await expect(client.pullWithReporter('invalid', reporter)).rejects.toThrow(
