...
describe('baseUrlToHostRoot', () => {
  it('converts OpenAI-compatible URLs to host root', () => {
    expect(baseUrlToHostRoot('http://localhost:11434/v1')).toBe('http://localhost:11434');
    expect(baseUrlToHostRoot('http://localhost:11434/v1/')).toBe('http://localhost:11434');
    expect(baseUrlToHostRoot('https://api.example.com/v1')).toBe('https://api.example.com');
  });

  it('leaves non-OpenAI URLs unchanged', () => {
    expect(baseUrlToHostRoot('http://localhost:11434')).toBe('http://localhost:11434');
    expect(baseUrlToHostRoot('http://localhost:11434/')).toBe('http://localhost:11434');
  });
