...
describe('Message History Protocol Types', () => {
  describe('HistoryEntry', () => {
    ...
    it('serializes to JSON correctly', () => {
      ...
      expect(json).toBe('{"conversation_id":"conv-456","ts":1699564900000,"text":"Test message"}');
    });

    it('deserializes from JSON correctly', () => {
      const json = '{"conversation_id":"conv-789","ts":1699565000000,"text":"Another message"}';
    ...
    it('handles special characters in text', () => {
      ...
      const json = JSON.stringify(entry);
      const parsed: HistoryEntry = JSON.parse(json);
      expect(parsed.text).toBe('Special chars: "quotes", \'apostrophes\', & ampersand, <html>');
    });
