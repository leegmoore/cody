...
describe('Plan Tool Protocol Types', () => {
  describe('StepStatus', () => {
    ...
    it('has exactly 3 status types', () => {
      const statusTypes = Object.keys(StepStatus).filter((k) => isNaN(Number(k)));
    ...
    it('can be used in switch statements', () => {
      ...
      expect(getStatusDescription(StepStatus.InProgress)).toBe('Currently executing');
  ...
  describe('PlanItemArg', () => {
    ...
    it('handles multiline step descriptions', () => {
      ...
      expect(parsed.step).toBe('Step 1:\nInstall dependencies\nVerify installation');
    });
  ...
  describe('UpdatePlanArgs', () => {
    ...
    it('creates a valid update plan args without explanation', () => {
      const args: UpdatePlanArgs = {
        plan: [
          { step: 'First step', status: StepStatus.Pending },
        ],
      };
    ...
    it('deserializes from JSON correctly', () => {
      const json = '{"explanation":"Build process","plan":[{"step":"Compile","status":"in_progress"}]}';
    ...
    it('handles complex multi-step plan', () => {
      ...
      const completedSteps = args.plan.filter(item => item.status === StepStatus.Completed);
      expect(completedSteps).toHaveLength(3);

      const inProgressSteps = args.plan.filter(item => item.status === StepStatus.InProgress);
      expect(inProgressSteps).toHaveLength(1);

      const pendingSteps = args.plan.filter(item => item.status === StepStatus.Pending);
    ...
    it('handles multiline explanation', () => {
      ...
      const json = JSON.stringify(args);
      const parsed: UpdatePlanArgs = JSON.parse(json);
      expect(parsed.explanation).toBe('Phase 1: Setup\nPhase 2: Build\nPhase 3: Deploy');
    });
