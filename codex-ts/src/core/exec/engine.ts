...
import { SandboxManager, type CommandSpec, type ExecEnv, SandboxType } from '../sandboxing/index.js';
import {
  DEFAULT_TIMEOUT_MS,
...
export function isLikelySandboxDenied(
  ...
  const outputs = [output.stderr.text, output.stdout.text, output.aggregatedOutput.text];
...
export async function processExecToolCall(
  ...
  const { command, cwd, timeoutMs, env, withEscalatedPermissions, justification } = params;
  ...
  const execEnv = manager.transform(spec, sandboxPolicy, sandboxType, sandboxCwd, codexLinuxSandboxExe);
...
function finalizeExecResult(
  ...
  if (timedOut) {
    throw new SandboxTimeoutError(`Command timed out after ${durationMs}ms`, output);
  }

  // Check for sandbox denial
  if (isLikelySandboxDenied(sandboxType, output)) {
    throw new SandboxDeniedError('Command was likely denied by sandbox', output);
  }
