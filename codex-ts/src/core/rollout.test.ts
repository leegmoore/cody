...
describe("rollout", () => {
  ...
  beforeEach(async () => {
    ...
    testCodexHome = await fs.mkdtemp(
      path.join(os.tmpdir(), "codex-rollout-test-"),
    );
  ...
  describe("RolloutRecorder.create", () => {
    ...
    it("should write session meta as first line", async () => {
      ...
      const recorder = await RolloutRecorder.create(testConfig, params);
      const lines = await RolloutRecorder.readRolloutHistory(
    ...
    it("should throw error when resuming non-existent file", async () => {
      ...
      await expect(
        RolloutRecorder.create(testConfig, params),
      ).rejects.toThrow();
    });
  });
  ...
  describe("archiveConversation", () => {
    ...
    it("should preserve file content when archiving", async () => {
      ...
      await recorder.recordItems([
        { type: "message", data: { text: "Keep this!" } },
      ]);
  ...
  describe("deleteConversation", () => {
    ...
    it("should throw error when deleting non-existent file", async () => {
      ...
      await expect(
        RolloutRecorder.deleteConversation(nonExistentPath),
      ).rejects.toThrow();
    });
  });
  ...
  describe("integration", () => {
    it("should handle complete conversation lifecycle", async () => {
      ...
      const archivedLines =
        await RolloutRecorder.readRolloutHistory(archivedPath);
    ...
    it("should preserve data integrity across operations", async () => {
      ...
      const testData = {
        message1:
          'First message with special chars: "quotes", newlines\n, and\ttabs',
