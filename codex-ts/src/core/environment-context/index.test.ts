...
describe('environment_context (simplified)', () => {
  describe('createEnvironmentContext', () => {
    ...
    it('should create context with all params', () => {
      const shell: Shell = { type: 'bash', shellPath: '/bin/bash', bashrcPath: '~/.bashrc' }
      const ctx = createEnvironmentContext(
        '/home/user',
  ...
  describe('equalsExceptShell', () => {
    ...
    it('should return true when only shell differs', () => {
      const shell1: Shell = { type: 'bash', shellPath: '/bin/bash', bashrcPath: '~/.bashrc' }
      const shell2: Shell = { type: 'zsh', shellPath: '/bin/zsh', zshrcPath: '~/.zshrc' }
  ...
  describe('serializeEnvironmentContext', () => {
    ...
    it('should serialize network_access', () => {
      const ctx = createEnvironmentContext(undefined, undefined, NetworkAccess.Enabled)
      const serialized = serializeEnvironmentContext(ctx)
    ...
    it('should serialize shell type', () => {
      const shell: Shell = { type: 'bash', shellPath: '/bin/bash', bashrcPath: '~/.bashrc' }
      const ctx = createEnvironmentContext(undefined, undefined, undefined, shell)
      const serialized = serializeEnvironmentContext(ctx)
      expect(serialized).toContain('shell: bash')
    })

    it('should serialize all fields', () => {
      const shell: Shell = { type: 'zsh', shellPath: '/bin/zsh', zshrcPath: '~/.zshrc' }
      const ctx = createEnvironmentContext(
        '/home/user',
        SandboxMode.WorkspaceWrite,
        NetworkAccess.Enabled,
