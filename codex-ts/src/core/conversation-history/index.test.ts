...
import { ConversationHistory, formatOutputForModelBody, MODEL_FORMAT_MAX_BYTES, MODEL_FORMAT_MAX_LINES } from './index'
...
describe('ConversationHistory', () => {
  ...
  describe('Basic Operations', () => {
    ...
    it('should remove ghost snapshots in getHistoryForPrompt', () => {
      const history = new ConversationHistory()
      history.recordItems([ghostSnapshot(), userMsg('hello'), assistantMsg('hi')])
  ...
  describe('removeFirstItem', () => {
    it('should remove oldest item', () => {
      const history = new ConversationHistory()
      history.recordItems([userMsg('first'), userMsg('second'), userMsg('third')])
      ...
      expect(items).toHaveLength(2)
      if (items[0].type === 'message') {
        expect(items[0].content[0]).toEqual({ type: 'input_text', text: 'second' })
      }
    })
