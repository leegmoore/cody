  ...
  describe('user messages', () => {
    it('should parse user message with text content', () => {
      ...
      if (result?.type === 'user_message') {
        ...
        expect(result.item.content[0]).toEqual({ type: 'text', text: 'Hello, world!' })
    ...
    it('should parse user message with image content', () => {
      ...
      if (result?.type === 'user_message') {
        ...
        expect(result.item.content[0]).toEqual({ type: 'text', text: 'Check this image:' })
    ...
    it('should filter out user messages with session prefix', () => {
      const item: ResponseItem = {
        type: 'message',
        role: 'user',
        content: [{ type: 'input_text', text: '<!-- session --> Internal message' }],
      }
  ...
  describe('agent messages', () => {
    it('should parse agent message with text content', () => {
      ...
      if (result?.type === 'agent_message') {
        expect(result.item.content).toHaveLength(1)
        expect(result.item.content[0]).toEqual({ type: 'text', text: 'Here is my response.' })
        expect(result.item.id).toBeDefined()
      }
    })
  ...
  describe('reasoning items', () => {
    it('should parse reasoning with summary', () => {
      ...
      if (result?.type === 'reasoning') {
        expect(result.item.summary_text).toEqual(['I need to analyze this problem'])
        expect(result.item.id).toBeDefined()
      }
    })
    ...
    it('should parse reasoning with summary and content', () => {
      const item: ResponseItem = {
        ...
        content: [{ text: 'Raw reasoning step 1' }, { text: 'Raw reasoning step 2' }],
      }
      ...
      if (result?.type === 'reasoning') {
        ...
        expect(result.item.raw_content).toEqual(['Raw reasoning step 1', 'Raw reasoning step 2'])
      }
    })
