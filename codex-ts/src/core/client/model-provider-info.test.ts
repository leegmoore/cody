...
describe('model-provider-info', () => {
  ...
  describe('ModelProviderInfo', () => {
    ...
    it('should create a provider with env_key', () => {
      ...
      expect(provider.envKeyInstructions).toBe('Get your key from https://example.com')
    })
  ...
  describe('builtInModelProviders', () => {
    ...
    it('should include oss provider', () => {
      ...
      expect(providers[BUILT_IN_OSS_MODEL_PROVIDER_ID].wireApi).toBe(WireApi.Chat)
      expect(providers[BUILT_IN_OSS_MODEL_PROVIDER_ID].requiresOpenaiAuth).toBe(false)
    })
    ...
    it('should have correct structure for openai provider', () => {
      ...
      expect(openai.envHttpHeaders?.['OpenAI-Organization']).toBe('OPENAI_ORGANIZATION')
  ...
  describe('createOssProvider', () => {
    ...
    it('should create OSS provider with custom base URL', () => {
      const provider = createOssProviderWithBaseUrl('https://custom.example.com/v1')
