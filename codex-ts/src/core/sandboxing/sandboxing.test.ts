...
describe('SandboxManager', () => {
  ...
  describe('selectInitial', () => {
    ...
    it('should forbid sandbox when preference is Forbid', () => {
      const result = manager.selectInitial(readOnlyPolicy, SandboxPreference.Forbid);
      expect(result).toBe(SandboxType.None);
    });

    it('should require sandbox when preference is Require', () => {
      const result = manager.selectInitial(dangerPolicy, SandboxPreference.Require);
    ...
    it('should use no sandbox for danger-full-access with Auto preference', () => {
      const result = manager.selectInitial(dangerPolicy, SandboxPreference.Auto);
      expect(result).toBe(SandboxType.None);
    });

    it('should use platform sandbox for read-only with Auto preference', () => {
      const result = manager.selectInitial(readOnlyPolicy, SandboxPreference.Auto);
    ...
    it('should use platform sandbox for workspace-write with Auto preference', () => {
      const result = manager.selectInitial(workspaceWritePolicy, SandboxPreference.Auto);
  ...
  describe('transform', () => {
    ...
    it('should transform with no sandbox', () => {
      const result = manager.transform(baseSpec, readOnlyPolicy, SandboxType.None, '/workspace');
    ...
    it('should not add network disabled flag for workspace-write with network', () => {
      ...
      expect(result.env[CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR]).toBeUndefined();
    });

    it('should not add network disabled flag for danger-full-access', () => {
      const result = manager.transform(baseSpec, dangerPolicy, SandboxType.None, '/workspace');
    ...
    it('should throw error for Linux sandbox without executable path', () => {
      expect(() => {
        manager.transform(baseSpec, readOnlyPolicy, SandboxType.LinuxSeccomp, '/workspace');
    ...
    it('should not mutate original spec environment', () => {
      ...
      manager.transform(baseSpec, readOnlyPolicy, SandboxType.None, '/workspace');
  ...
  describe('Integration scenarios', () => {
    it('should handle full workflow for read-only command', () => {
      ...
      const env = manager.transform(spec, policy, sandbox, '/project', '/usr/bin/codex-sandbox');
