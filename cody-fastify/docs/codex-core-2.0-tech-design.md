...
The codebase is organized as a monorepo, but we are shifting the center of gravity:

*   **`cody-fastify` (The New Home):**
    *   **Role:** The Application Server AND the location of the **New Core 2.0**.
    *   **Contents:**
        *   `src/core/`: The new Streaming Core (Adapters, Redis Loop, Zod Types).
        *   `src/api/`: Fastify Routes & SSE.
        *   `src/workers/`: Projectors and Background Jobs.
    *   **Constraint:** This is where ALL new code is written.

*   **`codex-ts` (Legacy / Deprecated):**
    *   **Role:** A bucket of legacy tools.
    *   **Contents:** `apply-patch`, `fs` tools, `sandboxing`.
    *   **Status:** **DEPRECATED.** The CLI is dead. Do not write new features here. Only import specific utility functions (like the patcher) into `cody-fastify` as needed. Eventually, valuable tools will be migrated out and this package will be deleted.

### 2.4. Environment Configuration
*   **Location:** `.env` configuration is managed exclusively in `cody-fastify`.
*   **State:** The environment is **already configured** with necessary keys (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `LANGFUSE_PUBLIC_KEY`, etc.).
*   **Rule:** Do NOT create new `.env` files.

### 2.5. Testing Strategy: NO MOCKS
*   **Absolute Rule:** We do **NOT** mock infrastructure.
*   **Redis:** Tests must run against the real local Redis instance.
*   **LLMs:** Tests must make real network calls to OpenAI/Anthropic (or use a record/replay proxy if strictly necessary, but never a logic-less mock).
*   **Why:** We are testing the *pipeline*, not the unit. Mocking the Redis client hides the serialization bugs that plague streaming architectures.
*   **Tooling:** Use **Playwright** for E2E flows. Use standalone scripts for infrastructure verification.
